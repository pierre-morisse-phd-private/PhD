...
int main(int argc, char* argv[]) {
	...
	fprintf(stderr, "MINEXT :%d\n", minExt);
	fprintf(stderr, "MAXOV :%d\n", maxOv);
	...
	while (line != NULL) {
		...
		while (line != NULL && strcmp(q1id, q2id) == 0) {
			...
			int written = 0;
			if (strcmp(r1id, r2id) != 0 && ((gap >= minGap && gap <= maxGap) || (gap <= 0 && gap >= maxOv && gap + r2len >= minExt && getNodeLength(r1id, strlen(r1id)) > -gap))) {
				written = 1;
			...
			if (written || gap > maxGap || score2 > score1) {
			//~ if (1) {
				size1 = size2;
				score1 = score2;
				//~ nbLinks++;free(r1id);
