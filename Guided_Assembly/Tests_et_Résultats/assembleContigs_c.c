...
char* get_subsequence(char* id, char* file, int pos, int len) {
	...
	//~ printf("%s ; size = %d ; from %d to %d\n", id, strlen(line), pos, pos + len);
...
char* getConsensus(char** sequences, int size) {
	char* res = malloc(strlen(sequences[0]) + 1);
	
	if (size < 3) {
		strncpy(res, sequences[0], strlen(sequences[0]) + 1);
		return res;
	}
...
void getLongestExtension(graph_t graph, int n, adj_list_node_t node, char** contigs, char* LR_file, char* contigs_file, int* visited, scaff_res_t* scaffolds,
	...
	//~ fprintf(stderr, "DEBUT BOUCLE\n");
		...
		*distance = *distance + get_distance(contigs[n], contigs[node2->dest], al_file);
		...
		if (node2->gap > 0) {
			...
			strncat(res, node2->LR + node2->pos, node2->gap);
			if (n == 64) {
				//~ fprintf(stderr, "|%s|\n", res);
				printf("gap : %d ; consensus : %d\n", node2->gap, strlen(node2->LR));
			}
			//~ printf("J'ai bien concatene le consensus\n");
		}
		...
		node = node2;
		//~ fprintf(stderr, "JOINTURE CONTIG GAP CONTIG DOINE\n");
		if (node != NULL) {
			node = node->next;
			while (node != NULL) {
				l1 = getNodeLength(contigs[node2->dest], strlen(contigs[node2->dest])) + node2->gap;
	...
	scaffolds[n]->scaffold[strlen(res)] = '\0';
	free(res);
	//~ return res;
}
...
int main(int argc, char* argv[]) {
	...
	while (line != NULL) {		
		...
		if (nb_src == src_prec && nb_dest == dest_prec) {
			seq = get_subsequence(LR, argv[3], pos, len);
			if (links_size <= i_links && seq != NULL) {
				//~ printf("I REALLOC : %d\n", links_size);
				//~ printf("I GIVE : %d\n", 2 * links_size);
			...
			addEdge(graph, src_prec, len, 0, consensus, dest_prec);
			//~ printf("ADD EDGE DONE\n");
			src_prec = nb_src;
			dest_prec = nb_dest;
			for (int i = 0; i < i_links; i++) {
				free(links[i]);
			}
			free(links);
			//~ printf("FREE DONE\n");
