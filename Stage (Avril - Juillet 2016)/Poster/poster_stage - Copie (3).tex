...
\myfig{
	\begin{tikzpicture}
		...
		% Flèche
		\draw [-triangle 45] (4.25,-1.5) -- (4.25,-3);
		...
		% Seed
		\draw [|-|] (2,-5) -- (5,-5);
		\draw [|-|] [green] (5,-5) -- (6.5,-5);
		\node [right] at (6.5,-5) {\tiny \emph{seed} mis à jour};		
	\end{tikzpicture}
	\figcaption{Illustration du processus de recrutement de \emph{reads} similaires pour un \emph{seed} donné. $lmin$ représente le seuil 
	permettant de déterminer la similarité. Ici, on remarque que le \emph{seed} est similaire à $r2$, mais pas à $r1$. $r2$ est donc recruté, et
	le \emph{seed} est mis à jour avec les parties de ce \emph{read} ne le chevauchant pas, afin d'étendre sa longueur.}
	\label{figrec}
}
