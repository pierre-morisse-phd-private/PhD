...
\SetBgContents{
	\begin{pspicture}(180,180)
		\psline[linewidth=1mm](118.5,66.95)(118.55,163.45)
		\psline[linewidth=1mm](148.5,66.95)(148.4,149.8)
	\end{pspicture}
}
...
\begin{center}
	...
	\begin{tabular}{ccc}
		\begin{minipage}[c]{0.35\textwidth}
			...
			\flushleft
			\vspace*{-9cm}
			\includegraphics[width=0.35\textwidth]{NU.eps} \\
			\includegraphics[width=0.35\textwidth]{logo.eps}
		\end{minipage} &
		
		\begin{minipage}[c]{0.25\textwidth}
			\centering
			\textsl{\LARGE Pierre Morisse \\ Master IGIS spécialité ITA \\ 2° année \\ 2015 - 2016 \\}
		\end{minipage} &
		...
		\begin{minipage}[c]{0.35\textwidth}
			...
			\flushright
			\vspace*{-9.7cm}
			\includegraphics[width=0.35\textwidth]{UFR.eps} \\
			\vspace{4.2cm}
			\includegraphics[width=0.35\textwidth]{logol.eps}
		\end{minipage}
	\end{tabular}
...
Depuis le milieu des années 2000 et le développement des séquenceurs à très haut débit (\emph{Next Generation Sequencing}), la biologie doit faire face au traitement d'énormes quantités de données, formées par des millions de très courtes séquences appelées \emph{reads}. Ces \emph{reads} sont utilisés pour le traitement de problèmes de \emph{mapping} ou d'assemblage, et doivent souvent subir une procédure de correction avant utilisation. Philippe et al. ont souligné, dans \cite{Philippe2011}, l'importance de l'indexation des \emph{reads} afin de résoudre ces problèmes, et ont développé un index supportant les 7 requêtes suivantes, pour une séquence $f$ de longueur $k$ donnée : \bigskip
...
\mytab{
	\resizebox{\columnwidth}{!}{ 
		\begin{tabular}{|c|c|c|c|c|c|}
			\hline
			Nombre de \emph{reads} & Longueur moyenne & Contigs produits par \emph{read} & Longueur moyenne & Précision moyenne & 
			Plus long contig (précision) \\
			\hline
			463 & 4 078 & 2,42 & 623 & 89,90 & 3 728 (95 \%) \\
			\hline
			322 & 9 619 & 3,17 & 1 495 & 87,78 & 17 797 (94 \%) \\
			\hline
			1 713 & 1 549 & 2,89 & 742 & 87,94 & 15 223 (91 \%) \\
			\hline
			2 871 & 11 583 & 2,41 & 4 349 & 88,83 & 33 317 (91 \%) \\ 
			\hline
		\end{tabular}
	}
	\tabcaption{Résultats obtenus par notre méthode sur différents ensembles de données.}
	\label{resresres}
}

Notre méthode permet donc de produire des contigs assez longs et précis, et semble constituer un prétraitement efficace pour la production de \emph{reads} NaS. Il serait nécessaire d'étudier les contigs produits, afin de voir s'ils se chevauchent et peuvent ainsi être fusionnés, afin de produire, pour chaque \emph{read} long, un unique contig de longueur plus importante, et donc un éventuel \emph{read} NaS.
